...
import { CURRENCIES, PHONEPREFIX, TITLES } from '@/components/constants/settings';
...
export const FormInput = ({
  ...
  const BASE_CURRENCY = countries?.find(
    (item) => item?.abbreviation === formik?.values?.currency
  ...
  return (
    <FormControl {...themeStyles.textStyles.sl5}>
      {label && <Text fontSize={'16px'} color={isDarkMode ? 'text' : 'matador_text.500'} pb={2}>
        {label}
      </Text>}
        ...
        {rest.type === 'amount' ? (
          ...
          >
            <HStack spacing="10px" pl="12px" w="full" h="full">
              <Text fontSize="15px" color={theme.theme_name !== 'light' ? "matador_text.500" : "matador_text.300"} fontWeight="400">
                {currency}
              </Text>
            </HStack>
            <Select
