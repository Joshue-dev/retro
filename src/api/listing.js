...
import {BaseURL_TWO, BaseURL_ONE, storeName, BUSINESS_ID, store_name} from '../constants/routes.js';
...
export const fetchProjectsWithFilters = async queryString => {
  return await axios.get(
    `${BaseURL_ONE}/investment/project?store_name=${storeName}${queryString}`);
};
...
export const fetchForCustomerEquityValidation = async () => {
  ...
  const businessId = JSON.parse(business_id)
...
export const fetchScheduleInspectionDetails = async (id) => {
  const business_id = await getSession("businessId");
  return await axios.get(
    `${BaseURL_ONE}/investment/scheduletour/${parseInt(
      id
...
export const getAllContactPersons = async projectId => {
 ...
 return await axios
    .get(`${BaseURL_TWO}/developers/contact-persons?project=${projectId}`)
