...
export const fetchPaymentPlanDoc = async (param) => {
  return await axios.get(
    `${BaseURL_TWO}/developers/project-documents?${param}`
  );
};
...
export const fetchAllListingBundles = async (projectId) => {
  return await axios.get(
    `${BaseURL_TWO}/investment/project-bundles/?project_id=${projectId}`
  );
};
...
export const scheduleInspection = (body) => {
  return axios.post(
    `${BaseURL_ONE}/investment/scheduletour/${projectId}`,
    body
  );
};
