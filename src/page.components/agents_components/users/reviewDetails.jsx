...
import { respondToInspectionFeedBack } from "@/api/customers";
...
const ReviewDetails = ({ reviewData, setOpenDetails }) => {
  ...
  const mutation = useMutation(
    (formBody) => respondToInspectionFeedBack(formBody, reviewData.id),
    {
      onSuccess: () => {
        formik.resetForm();
      ...
      <Stack my="0.5rem">
        <HStack>
          {[...Array(5)].map((_, index) => (
            <RatingIcon
              key={index}
              fill={index < reviewData.star_rating ? "#FF9103" : "#CBCBCB"}
            />
          ))}
        </HStack>
      </Stack>
