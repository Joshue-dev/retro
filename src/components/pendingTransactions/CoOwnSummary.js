...
const CoOwnSummary = ({
  ...
  const coownerInfo = coowners?.length
    ...
    : null;
  const sharedUnitPrice = calculateSharedValue(
    asset?.total_unit_price,
  ...
  const loggedInCoownerInfo = useQuery(
    ["coownerInfo", LoggedinUser?.user?.id, asset?.id],
  ...
  const paymentPlanIndividualPercentage =
    (Number(loggedInCoownerData?.user_amount_paid) /
      Number(sharedInitialDeposit)) *
    100;
  const paymentPlanGroupPercentage =
    (Number(asset?.amount_paid) /
      Number(asset?.payment_plan?.initial_deposit_in_value)) *
    100;
  const outrightIndividualPercentage =
    (Number(loggedInCoownerData?.user_amount_paid) / Number(sharedUnitPrice)) *
    100;
  ...
  const handleProceed = () => {
    if (!asset?.offer_started && asset?.owner?.id !== LoggedinUser?.user?.id) {
      toaster(
        `Before you can proceed, the Host needs to initialize a payment.`
      );
      ...
      setAmountToPay(
        asset?.payment_plan ? sharedInitialDeposit : sharedUnitPrice
      );
      setType("payment");
    }
  };
          ...
          {asset?.offer_started ? (
            ...
            >
              ...
              <Text color="text" mt="-2px" fontSize="11.448px" fontWeight="300">
                The payment made is still below the required initial deposit If
                the payment is not made by other co-owners before the exp. date
                or in the event of a price change, the amount already paid will
                be refunded.
              </Text>
            </HStack>
          ) : null}
