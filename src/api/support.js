...
export const initiateSupport = async (body) => {
  ...
  return axios.post(`${BaseURL_TWO}/supports/${businessId}/initiate/`, body);
};
...
export const fetchSupportEvents = async (id) => {
  ...
  await axios
    .get(`${BaseURL_TWO}/supports/event/${id}/`)
    .then((res) => (response = res));
  return response;
};
...
export const getSupportInitiation = (business_id) => {
  return axios.get(`${BaseURL_TWO}/supports/${business_id}/initiate/`);
};
