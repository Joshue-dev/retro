...
const CoOwnersList = ({
  ...
  const totalValue =
    asset?.payment_plan?.initial_deposit_in_value ?? asset?.total_unit_price;
  const excludeHostFromCoowners = coowners?.filter(
    (info) => info?.id !== theHost?.id
  );
              ...
              >
                ...
                <VStack align={"flex-end"} spacing={"12px"}>
                  <Text fontSize={"13px"} fontWeight={400}>
                    {formatToCurrency(
                      calculateSharedValue(theHost.equity_value, totalValue)
                    )}
                  </Text>
              ...
              {excludeHostFromCoowners?.map((user) => (
                ...
                >
                  ...
                  <VStack align={"flex-end"} spacing={"12px"}>
                    <Text fontSize={"13px"} fontWeight={400}>
                      {formatToCurrency(
                        calculateSharedValue(user.equity_value, totalValue)
                      )}
                    </Text>
                    <Box px="10px" py="3px" bg={"#F0F0F0"}>
                      <Text
