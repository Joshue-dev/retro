...
export const changeDateFormat = (string, extra) => {
  try {
    ...
    if (extra === 'full_month') {
      options = {hour: 'numeric', minute: 'numeric', hour12: true, month: 'long'};
    }
    if (extra === 'm/d') {
      options = {month: 'short'};
...
export const dayMonthYearSlash = date => {
  return moment(date).format('DD/MM/YYYY');
};
...
export const formatDateString = numericValue => {
...
export const isValidDate = (day, month, year) => {
  const date = new Date(`${year}-${month}-${day}`);
    if (year < 1000 || year > 9999) {
      return false; // Year should be within a reasonable range
    }
  
  return (
    date && date.getMonth() + 1 == month && date.getDate() == day && date.getFullYear() == year
  );
};
