...
import miniCalendarIcon from "/src/images/icons/agent_req_calendar.svg";
...
const Filter = ({ setUrl, url }) => {
  ...
  const handleChange = (e) => {
    ...
    return (
      ...
      setDate({ ...date, [e.target.name]: inputDate.toISOString() })
    );
  };
  const status = ["Virtual", "In-Person"];

  const isValid =
    Object.entries(date)?.length || toBeFiltered?.inspection_type?.length < 2;
