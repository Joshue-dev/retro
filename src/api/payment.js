...
export const fetchFractionsPurchaseHistory = async (id) => {
  return await axios.get(
    `${BaseURL_TWO}/investment/fraction-history/?equity_id=${parseInt(id)}`
  );
};
...
export const fetchAllPurchaseHistory = async (id) => {
  const USER_OBJECT = await getSession("loggedIn");
  return await axios.post(
    `${BaseURL_ONE}/transaction/equity/${parseInt(id)}/?display_fee=true`,
    {
      user_id: parseInt(USER_OBJECT?.user?.id),
    }
  );
};
...
export const fetchSavedCards = async () => {
  ...
  return await axios.get(
    `${BaseURL_TWO}/account/cards/?business_id=${businessId}`
  );
};
...
export const fetchBankDetails = async (id) => {
  return await axios.get(`${BaseURL_TWO}/payment/project-account/?project=${parseInt(id)}`);
}