...
export const fetchIndividualCoOwnersData = async (userID, equity_id) => {
  ...
  await axios
    .get(
      `${BaseURL_TWO}/investment/coown-individual/${equity_id}/?user=${userID}`
    )
...
export const expectedCoownersPayment = async ({ equityId }) => {
  return await axios.get(
    `${BaseURL_TWO}//investment/equity-payments/${equityId}/`
  );
};
...
export const fetchUserEquityInviteData = async ({ id }) => {
  return await axios.get(
    `${BaseURL_TWO}/v2/store/view_notification_data/${id}/`
  );
};
...
export const respondToCoOwnersRequest = async (body, coownerId) => {
  return axios.post(
    `${BaseURL_TWO}/investment/co-own/respond/${coownerId}/`,
    body
  );
};
...
export const proposeNewEquity = async (equityId) => {
  return axios.post(
    `${BaseURL_TWO}/investment/co-own/propose-new/${equityId}/`,
    { payment_option: "matador_wallet" }
  );
  // 	{
  //     "equity_value": 23
